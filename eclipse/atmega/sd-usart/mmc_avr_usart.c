...
#define	FCLK_SLOW()	UBRR0L = F_CPU/400000/2-1	/* Set SPI clock for initialization (100-400kHz) */
#define	FCLK_FAST()	UBRR0L = 0					/* Set SPI clock for read/write (20HMz max) */
...
void power_on (void)
{
	...
	UCSR0C = _BV(UMSEL01)|_BV(UMSEL00);	/* Set USART to SPI mode */
	UCSR0B = _BV(RXEN0)|_BV(TXEN0);		/* Enable USART */
}
...
void power_off (void)
{
	UCSR0C = 0;	/* Disable USART-SPI function */
...
{
	UDR0 = 0xFF;
	loop_until_bit_is_set(UCSR0A, UDRE0); UDR0 = 0xFF;
	cnt -= 2;
	do {
		loop_until_bit_is_set(UCSR0A, UDRE0);
		*p++ = UDR0; UDR0 = 0xFF;
	} while (--cnt);
	loop_until_bit_is_set(UCSR0A, RXC0); *p++ = UDR0;
	loop_until_bit_is_set(UCSR0A, RXC0); *p++ = UDR0;
}
...
{
	UDR0 = *p++;
	loop_until_bit_is_set(UCSR0A, UDRE0); UDR0 = *p++;
	cnt -= 2;
	do {
		loop_until_bit_is_set(UCSR0A, UDRE0);
		UDR0; UDR0 = *p++;
	} while (--cnt);
	loop_until_bit_is_set(UCSR0A, RXC0); UDR0;
	loop_until_bit_is_set(UCSR0A, RXC0); UDR0;
}
